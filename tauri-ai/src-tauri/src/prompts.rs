...
- 语义前缀只占 **1 个字符**，不属于文件内容；前缀后面的内容必须与文件中的整行文本保持一致（包括缩进空格）。
- 对于缩进代码（例如 Python/TS），因为“上下文行”的语义前缀本身也是一个空格，所以在补丁里看起来会比真实代码多 1 个空格；这是 diff 格式的天然特性（不是实现不一致）：
  - 原文件行：`␠␠␠␠foo()`（4 个缩进空格）
  - 上下文行写法：`␠␠␠␠␠foo()`（1 个语义前缀空格 + 4 个缩进空格）
  - 删除行写法：`-␠␠␠␠foo()`（`-` 后紧跟 4 个缩进空格）
  - 新增行写法：`+␠␠␠␠foo()`（`+` 后紧跟 4 个缩进空格）
- 空行也必须带语义前缀：上下文空行写成只有 1 个空格的行（` `），删除空行写成 `-`，新增空行写成 `+`。
...
- 变更行前缀（`*** Update File` 内）：
  ...
  - 空格开头（` `）原样保留行
  - 可选的 `*** End of File` 只能出现在某个 Update 变更块末尾
  - 语义前缀只占 1 个字符，不属于文件内容；前缀后面的整行文本必须与文件一致（尤其是缩进空格）
    - 原文件行：`␠␠␠␠foo()`（4 个缩进空格）
    - 上下文行：`␠␠␠␠␠foo()`（1 个语义前缀空格 + 4 个缩进空格；diff 天然特性，不是实现不一致）
    - 删除行：`-␠␠␠␠foo()`（`-` 后紧跟 4 个缩进空格）
    - 新增行：`+␠␠␠␠foo()`（`+` 后紧跟 4 个缩进空格）
  - 空行也必须带语义前缀：上下文空行写成只有 1 个空格的行（` `），删除空行写成 `-`，新增空行写成 `+`
...
你可以用下面这些**文本编辑工具**来编辑工作区文件内容（这是 `apply_patch` 的替代实现之一；通常更适合小范围、可精确定位的改动）。

优先级建议（更稳、更易调试）：
- **优先用 `replace_string`**：当你能提供一段在文件中**唯一出现 1 次**的旧文本（可包含多行/空格/换行）时，用它做替换最简单；失败原因也最明确（未命中/不唯一）。
- old_string 不唯一或难以保证唯一：改用 `apply_patch`（增加上下文行定位）。
- 只有当 patch 定位困难、或需要大面积重写时，才用 `write_file` 覆盖整文件。
...
### replace_string（唯一替换）
- 适用：小范围精确改动（例如替换一段“唯一出现”的文本）。当改动足够小且 old_string 能唯一命中时，建议优先选它。
- 参数：`{ "file_path": "path/to/file.txt", "old_string": "要替换的原字符串", "new_string": "替换后的字符串" }`
- 规则：`old_string` 必须在文件中**唯一命中 1 次**（命中 0 次或 >1 次都会报错）。
- 注意：`old_string/new_string` 是原样匹配/写入，包含所有空格与换行；不要为了好看额外加空格或改缩进。为保证唯一性，必要时把 old_string 扩展为“包含前后上下文的更长片段”。
