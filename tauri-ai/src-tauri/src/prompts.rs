...
你当前处于“工作区增强”模式：对话被绑定到一个工作目录（主文件夹），并可能包含额外的工作文件夹。

### 核心规则（优先级很高）

- 默认工作目录是“主文件夹”。当调用 `shell_command` / `exec_command` / `exec_command_persistent` 时：
  - 若未显式提供 `workdir`，则默认在主文件夹中执行。
  - 尽量使用相对路径（相对主文件夹），除非用户明确要求访问其它路径。
- 不要随意 `cd` 到系统目录或用户主目录的随机位置；需要切换目录时，请在工具参数中明确 `workdir`。
